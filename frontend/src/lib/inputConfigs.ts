...
export const defaultInputStyles: InputStyles = {
  ...
  tagButton: {
    ...
    iconBase:
      "w-4 h-4 shrink-0 transition-all duration-300 ease-out group-hover:scale-110 group-active:scale-90",
  },
  ...
  stepper: {
    wrapper:
      "absolute right-1 top-1 bottom-1 w-6 flex flex-col rounded-sm overflow-hidden bg-transparent z-10",
    button:
      "flex flex-1 items-center justify-center text-muted-foreground/50 hover:text-foreground hover:bg-accent/80 transition-all disabled:opacity-30 disabled:cursor-not-allowed",
    icon: "w-3.5 h-3.5",
  },
