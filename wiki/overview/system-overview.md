...
当前系统以 FastAPI + SQLite 作为事实源，前端负责业务操作和实时刷新，浏览器扩展只负责把外部购物车批次桥接到 `/cart-import`，CLI、MCP 和企业微信入口都通过后端 API 收口。
...
3. 自动化入口层：`lsm_cli/` 提供命令行访问，`lsm_mcp/` 通过 CLI 子进程暴露受控工具，`robot/` 处理企业微信智能机器人和微信客服消息。
...
5. 基础设施层：<InlineCodeRef href="https://github.com/hzb666/LabStorageManager/blob/main/app/database.py" /> 初始化 SQLite，<InlineCodeRef href="https://github.com/hzb666/LabStorageManager/blob/main/app/core/auth.py" /> 和 `redis.py` 处理认证与会话，`docker` 与 `nginx` 处理部署边界。
...
- `lsm_cli/`、`lsm_mcp/` 和 `robot/`：按受控命令面处理脚本、MCP、企业微信智能机器人和微信客服入口，写操作仍由后端权限控制。
...
- MCP 不直接访问数据库，也不开放 CLI 未显式暴露的 API。
