...
// 关闭态只在时效内有效；超时或公告更新后都要清掉旧记录，避免把新公告也一并藏掉。
...
export function AnnouncementBanner({
  ...
  const handleDismissAnnouncement = (id: number) => {
    ...
    // 当前会话也要立即从渲染结果里移除，避免等下一次 filter 才消失。
    setDismissedIds((prev) => [...prev, id]);
  };

  const renderBannerItems = (trackKey: string) =>
    visibleAnnouncements.map((announcement) => (
      <AnnouncementBannerItem
        key={`${trackKey}-${announcement.id}`}
        announcement={announcement}
    ...
    <>
      <div className="hidden md:flex flex-1 h-full overflow-hidden">
        <div className="marquee-root flex-1 h-full relative">
          <div className="marquee-content">{renderBannerItems("primary")}</div>
          <div className="marquee-content" aria-hidden="true">
            {renderBannerItems("secondary")}
          </div>
