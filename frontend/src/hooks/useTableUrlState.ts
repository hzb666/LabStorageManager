...
export function useTableUrlState(
  ...
  const initialColumnFilters: ColumnFiltersState = useMemo(() => {
    ...
    for (const cfg of columnFiltersCfg) {
      const raw = (search as Record<string, unknown>)[cfg.searchKey]
  ...
  const pagination: PaginationState = useMemo(() => {
    const rawPage = (search as Record<string, unknown>)[pageKey]
    const rawPageSize = (search as Record<string, unknown>)[pageSizeKey]
  ...
  const [globalFilter, setGlobalFilter] = useState<string | undefined>(() => {
    ...
    const raw = (search as Record<string, unknown>)[globalFilterKey]
  ...
  const ensurePageInRange = (
    ...
    const currentPage = (search as Record<string, unknown>)[pageKey]
