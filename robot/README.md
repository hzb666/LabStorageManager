...
查询、借用和归还都必须先完成企业微信用户绑定，之后所有 MCP 调用都使用该用户自己的 LabStorageManager token。借用和归还由受控流程处理：LLM 可理解用户意图和参数，但系统仍先检查绑定，再查候选，最后等待用户回复“确认”才执行。用户消息里包含“退出”“解绑”“登出”“注销”“换账号”等账号退出意图时，会发起手动解除绑定，并且必须再次回复“确认”才会真正解除。机器人发现已保存的用户 token 过期或认证失败时，会自动解除绑定并提示用户重新绑定。
