...
- [X] #1 生产环境密钥硬编码问题
- [X] #2 SQL注入风险 (CAS LIKE模式)
- [X] #3 使用 window.prompt()
- [X] #4 未实现的导出按钮
- [X] #5 订单入库数量验证缺失
- [X] #6 库存序号竞态条件
- [X] #7 用户创建接口无认证
...
- [X] #8 函数名冲突 get_inventory_by_code
- [X] #9 遗留 stock_in_order 使用错误的 generate_internal_code
- [X] #10 驳回原因未存储

### 全面代码审查重构 (14项)

- [X] #11 WAL 模式未正确启用
- [X] #12 generate_internal_code 双实现冲突
- [X] #13 用户注册接口无认证
- [X] #14 Dashboard 入库按钮状态逻辑
- [X] #15 reject/confirm-arrival 参数不匹配
- [X] #16 旧 stock-in 路由冗余
- [X] #17 订单列表接口无认证
- [X] #18 归还数量无上限校验
- [X] #19 路由顺序 + 函数名冲突
- [X] #20 CSV 导出返回 JSON
...
- [X] #23 Token 双重存储
- [X] #24 前端状态映射分散
- [X] #25 spec_utils 单位大小写
- [X] #26 /dashboard/my-borrows 时区问题

### PR Review 修复 (4项)

- [X] #27 pd.read_csv 参数错误
- [X] #28 驳回操作覆盖 notes 备注
- [X] #29 APPROVED 一键入库缺少权限检查
- [X] #30 common_public 订单可被错误入库
- [X] #31 切换每页条数后数据不变化
