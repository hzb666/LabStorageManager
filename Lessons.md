...
## 2026-07-22 网络字体等待必须限制页面隐藏时间

- 现象：网络字体等待过短会提前回退并在字体下载完成后改变字形，等待过长会让正文持续不可见。
- 原因：字体加载状态同时控制页面可见性和字体栈切换，超时值会直接影响首次访问体验。
- 做法：字体等待采用可控的短时窗口，覆盖常见加载延迟并保留失败回退；调整后同时验证首次加载和生产构建。
