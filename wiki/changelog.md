...
- 新增本地 CLI、MCP 工具服务、企业微信智能机器人和微信客服入口，外部消息入口统一回到后端 API。
- CLI 增加常用货架和 CAS 主数据查询能力，MCP 按白名单映射 CLI 命令，不直接访问数据库。
- 浏览器插件的 `manifest.json` 和 `shared/generated-config.js` 改为构建期产物，生产域名由插件 env 写入。
- 日志归档调度支持按服务器本地时间每天或每周执行，归档锁避免多 worker 重复运行。
- 生产上传请求上限对齐为 12MB，公告图片和头像仍保持 5MB 业务限制。

## 0.9.1

- 常用货架采用“分组 + 瓶级条目”模型，新增分组定位、位置建议、加瓶/扣减与条目级维护接口。
...
- 完善耗材订单模型与页面，增强筛选、导出和缓存能力。
- 增强试剂订单字段与校验逻辑，业务语义更加清晰。
- 打通浏览器插件购物车同步链路，外部采购清单可以直接进入系统导入流程。

## 0.5.0

- 完善库存模块，并补齐 `DataTable`、拼音排序和展开状态持久化。
