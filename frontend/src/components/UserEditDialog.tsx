...
function resetAvatarInput() {
  const fileInput = document.getElementById(AVATAR_INPUT_ID) as HTMLInputElement | null
  if (fileInput) {
    fileInput.value = ''
  }
}
...
/** 根据当前弹窗模式构建用户资料更新 payload。 */
function buildUpdatePayload(formData: UserUpdateFormData, mode: 'admin' | 'profile') {
...
function applyPasswordError(
  ...
  if (errorMessage.includes('Password must be at least') || errorMessage.includes('至少')) {
...
function useUserEditForms(open: boolean, user: User | null) {
  ...
  const [passwordEditSession, setPasswordEditSession] = useState<symbol | null>(null)
  const currentSessionToken = useMemo(
    () => (open && user ? Symbol('user-edit-session') : null),
    [open, user]
  )

  const isEditingPassword = currentSessionToken !== null && passwordEditSession === currentSessionToken
...
/** 管理头像预览、上传、删除以及本地 URL 的清理逻辑。 */
function useAvatarState(
  open: boolean,
  user: User | null,
  rememberedUser: ReturnType<typeof useRememberedUser>['rememberedUser'],
  updateRememberedUser: ReturnType<typeof useRememberedUser>['updateRememberedUser']
) {
  ...
  const handleAvatarDelete = useCallback((event: React.MouseEvent<HTMLButtonElement>) => {
  ...
  // 根据当前头像状态决定是删除头像、跳过上传还是执行真正的上传请求。
  const handleAvatarUpdate = useCallback(async (targetUser: User) => {
  ...
  }, [avatarFile, avatarPreview, originalAvatarUrl, rememberedUser?.userId, updateRememberedUser])
...
function useSaveUserHandler({
  ...
  return useCallback(async () => {
    ...
    try {
      ...
      const response = await userAdminAPI.update(user.id, buildUpdatePayload(formData, mode))
...
function AvatarSection({
  ...
  const avatarFallback = (avatarLoading || !avatarImageLoaded) && avatarPreview
    ? '...'
...
function RoleSelector({ role, onRoleChange }: Readonly<RoleSelectorProps>) {
  return (
    <div>
      ...
      <RadioGroup value={role} onValueChange={(value) => onRoleChange(value as UserRole)} className="flex gap-4 mt-2">
        {USER_ROLE_OPTIONS.map((option) => (
          <div key={option.value} className="flex items-center space-x-2">
            <RadioGroupItem value={option.value} id={`edit_role_${option.value}`} />
            <Label htmlFor={`edit_role_${option.value}`} className="text-base cursor-pointer">
              {option.label}
            </Label>
          </div>
        ))}
  ...
  return (
    <>
      <div className="grid space-y-4">
        {showOldPassword && <BaseForm form={passwordForm} fields={OLD_PASSWORD_FIELDS} layout="stack" />}
        <BaseForm form={passwordForm} fields={NEW_PASSWORD_FIELDS} layout="stack" />
      </div>
      <div key="password-actions" className="flex gap-3 mt-8">
        <Button variant="modern" onClick={onBack} size="lg" className="flex-1">
          返回
        </Button>
        <Button onClick={onSubmit} disabled={changePasswordLoading} size="lg" className="flex-1">
  ...
  return (
    <>
      ...
      <div key="edit-actions" className="flex gap-2 mt-6">
        <Button variant="modern" onClick={onStartPasswordEdit} size="lg" className="flex-3">
        ...
        <LoadingButton onClick={onSave} isLoading={editLoading} size="lg" className="flex-1">
          保存
        </LoadingButton>
      </div>
    </>
