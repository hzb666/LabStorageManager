...
这条链路的重点不是“能抓到数据”，而是“插件只是桥接，最终仍走系统自身的订单创建规则”。
...
- `useListSSE` 对不影响搜索和排序语义的更新执行安全 patch；其他更新会标记 stale 并触发重拉。
- `public` 账号在建单、导入等写入场景下受到额外限制。
- `/cart-import` 导入页主链路逐条调用标准订单创建接口。
- 输入校验由前端 schema、后端 DTO 和服务端标准化共同承担。
