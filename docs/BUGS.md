...
## 2026-03-12 Google Fonts 中科大镜像加载失败

### 问题描述

```
GET https://fonts.proxy.ustclug.org/css2?family=Noto+Sans+SC:wght@400;700&display=swap 
...
| 序号 | 可能原因                                                     | 可能性     |
| ---- | ------------------------------------------------------------ | ---------- |
| 1    | 中科大镜像服务停止 -`fonts.proxy.ustclug.org` 服务器已关闭 | ⭐⭐⭐⭐⭐ |
| 2    | 网络/代理问题 - 用户所在网络环境无法访问该域名               | ⭐⭐⭐     |
| 3    | DNS 解析问题 - DNS 服务器无法正确解析该域名                  | ⭐⭐       |
| 4    | 防火墙/GFW 干扰 - 被防火墙拦截                               | ⭐⭐       |

### 诊断结论

1. **中科大镜像服务已停止** - `ERR_CONNECTION_CLOSED` 表示连接被服务器直接关闭，这是服务器端停止服务的典型表现
2. 用户可能需要使用 VPN 或代理才能访问 Google Fonts 官方源

### 修复方案

1. 保留中科大镜像作为首选（国内访问速度快）
2. 添加 Google Fonts 官方源作为后备

### 修复文件

- `frontend/index.html` - 添加 Google Fonts 官方源作为后备

### 状态

- [X] 已修复 - 添加 Google Fonts 官方源作为后备，优先使用中科大镜像

---

## 2026-03-13 首屏渲染被在线字体加载阻塞

### 问题描述

- 页面入口依赖 `frontend/index.html` 中同步的字体样式表。
- 当 `Noto Sans SC` 响应慢或网络不稳定时，浏览器会推迟首屏可见内容渲染，表现为“必须等字体加载完才能进入网页”。

### 根本原因

1. `head` 中使用同步 `<link rel="stylesheet">` 加载远程字体 CSS，首屏会受外链样式返回速度影响。
2. 页面默认字体栈直接包含在线字体，缺少“先系统字体、后远程字体”的显式切换策略。

### 修复方案

1. 将在线字体改为前置脚本异步注入，不再阻塞首屏。
2. 首屏最多等待 1 秒远程字体完成，1 秒内若已就绪则直接用远程字体首次渲染。
3. 超过 1 秒仍未就绪时，再显示页面并使用系统字体栈。
4. 在线字体后续加载完成时，再切换到 `Noto Sans SC`。
...
- [X] 已修复 - 首屏不再被字体请求阻塞，字体可在加载完成后自动切换

---

## 2026-03-13 全局 500 响应泄露内部异常信息

### 问题描述

- 全局异常处理器把 `str(exc)` 直接返回给前端。
- 任意触发 500 的请求都可能看到 SQL 错误、文件路径或第三方库异常文案。

### 根本原因

1. `app/main.py` 中 `global_exception_handler` 同时承担了记录日志和组装客户端返回值的职责。
2. 在返回 JSON 时直接拼接了异常文本，导致内部实现细节泄露到 HTTP 响应体。

### 修复方案

1. 保留服务端完整异常日志与 traceback。
2. 前端只返回固定文案 `Internal server error`，避免泄露内部细节。
...
- [X] 已修复 - 服务端仍保留详细日志，客户端不再收到原始异常文本


## 2026-03-13

- `frontend/src/hooks/useTableState.tsx`
  React Hooks lint 规则 `set-state-in-effect` 命中过去的 `grandTotal` 同步写入逻辑。修复方式是移除 effect 内的同步 `setState`，改为从 React Query 的基础列表缓存直接推导总数，避免级联渲染。
